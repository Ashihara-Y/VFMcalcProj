...
# 長めの行数と必要な列数で、DFを生成して、各カラムはそれぞれ必要な行数のSeriesで生成し、それを
# 『DFの「適切な位置」に配置する』という作業にできないか？　
# Excelはこうした形で進められるところが、カラムごとに始期・終期がまちまちになっている、こうした
# ケースに向いている。Pythonでもそうした形で実現できないか？

# 参照用のカラムを数通り用意する。参照用に０，１，２，３，等を適宜、事業期間、整備期間、元本返済
# 据置期間、地方債償還期間、モニタリングコストや開業費用、整備費用、補助金、交付金、記載額の入金
# 時期等に応じて設定して、入れておく。それぞれのカラムに、必要なデータ(別途のリスト内で生成して
# おく)を入れていく。その後、それぞれのカラムを結合して、DFを作成する。
# この方法であれば、カラムごとにデータを入れていくので、データの整合性が取りやすい。
