...
# 長めの行数と必要な列数で、DFを生成して、各カラムはそれぞれ必要な行数で生成し、それを『DFの「適切な位置」に配置する』
# という作業にできないか？　Excelはこうした形で進められるところが、カラムごとに始期・終期がまちまちになっている、こうした
# ケースには向いている。Pythonでもそうした形で実現できないか？
