...
・スライダーの移動が停止してから0.2秒（？）後に、その変更値（複数の可能性があることに注意！）を反映した「編集後の入力データ」が
　抽出・計算に通した後の辞書を、VFM算定過程（Editcalc？）に渡す　⇒
・編集後のVFM算定結果を、インメモリのDBに書き込む（Editcalcの最後の関数に、インメモリ保存のための引数を渡す）　⇒
・DBから、編集後のVFM算定結果のうち、「編集後の算定結果要約」と「編集後の入力値」を抽出する。この部分のコードがあったか？　⇒
・前者は「編集後の算定結果」表の更新へ、後者は元のUI部品で、スライダーの位置だけ入力した値の場所にするのに使う（「元に戻せる」
　観点から、UIは元のままで値だけ最後の入力値のままにする）　⇒
「算定結果保存がクリックされた後」
・「この算定結果を保存」（仮）ボタンをクリックすると、その時点での「編集後算定結果」（インメモリＤＢ上に残っている）を抽出して、
 　ファイルＤＢに書き込む。　⇒
