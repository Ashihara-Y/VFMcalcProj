...
class Results(ft.Stack):
    def __init__(self, selected_datetime):
        ...
        # 以下のコードでは、算定過程で、IIとFIを使ってセットした初期値等が、JSON2つから再現される。
        # 各算定結果に、その初期値等をセットで保存するためには、このコードで問題ない。
        # ただし、この画面の「入力値等一案」タブで表示したいのは、結果一覧で選択された算定結果の初期値等で、
        # 「今しがた計算した結果の初期値等」ではない。
        # 本来、算定で使った初期値等を再現してDBに保存する処理は、このクラスの責務として不自然。
        # この初期値等の再現とDBへの保存を別モジュールに切り出して、VFMcalc()のところで呼んで実行する方が良い整理。
        ...
        #final_inputs_df.from_sql('final_inputs_res_table', engine, )
