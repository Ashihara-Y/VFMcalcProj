...
from final_inputs_models import InitialInputsData
...
async def main(page: ft.Page):
    ...
    def route_change():
        ...
        if page.route == "/final_inputs":
            raw_data = page.session.store.get("initial_inputs")
            if not raw_data:
                page.views.append(
                    ft.View(
                        route="/final_inputs",
                        controls=[
                            ...
                            ft.Text("初期データがありません。最初の画面（初期入力）に戻って処理をやり直してください。"),
                            ft.Button(content="最初に戻る", on_click=lambda e: page.push_route("/"))
                        ],
                        scroll=ft.ScrollMode.ALWAYS,
                    )
                )
            else:
                initial_data = InitialInputsData(**raw_data)
                page.views.append(
                    ft.View(
                        route="/final_inputs",
                        controls=[
                            ft.AppBar(title=ft.Text("入力確認と追加入力")),
                            Final_Inputs.init(initial_data=initial_data)
                            #ft.Button(content="計算", on_click=open_saved_list),
                        ],
                        scroll=ft.ScrollMode.ALWAYS,
                    )
                )
